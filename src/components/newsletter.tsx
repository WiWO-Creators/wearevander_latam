...
export function Newsletter({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section
      ...
      className={
        compact
          ? "border border-ink bg-ink px-5 py-6 text-paper"
          : "border-t border-ink bg-ink px-4 py-10 text-paper sm:px-6"
      }
    >
      <div className={compact ? "" : "mx-auto max-w-7xl lg:flex lg:items-end lg:justify-between lg:gap-10"}>
        <div>
          ...
          <h2 className="headline mt-2 text-4xl uppercase sm:text-5xl">
            Un mail. Cero feed.
          </h2>
          <p className="mt-2 max-w-xl font-body text-sm leading-relaxed text-paper/75">
            La mesa regional resume la semana: México, Brasil, Argentina, Colombia, Chile, Perú.
            We Are Vander. We Love Business.
          </p>
        </div>
        ...
        {status !== "idle" && (
          <p className={`mt-3 font-display text-sm ${status === "ok" ? "text-paper/80" : "text-rust"}`}>
            {message}
          </p>
        )}
      </div>
