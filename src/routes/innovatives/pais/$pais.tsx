...
import { innovCountry, innovCountries } from "@/lib/innovatives";
import { InnovativesMark } from "@/components/brand";
import { Newsletter } from "@/components/newsletter";
import { CatChip, EmptyCat, RankRow50 } from "@/components/rank-pack";

export const Route = createFileRoute("/innovatives/pais/$pais")({
  component: InnovCountryPage,
  head: ({ params }) => {
    const group = innovCountry(params.pais);
    return {
...
function InnovCountryPage() {
  const { pais } = Route.useParams();
  const group = innovCountry(pais);
  const all = innovCountries();
  if (!group) {
    return (
      <EmptyCat>
        <p className="kicker text-xs text-innov">Fuera de lista</p>
        <h1 className="headline mt-3 text-3xl">Ese país no está en las 50 Innovatives.</h1>
    ...
    <main>
      <section className="bg-ink px-4 py-12 text-paper sm:px-6 sm:py-16">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="kicker mt-6 text-xs text-innov">50 Innovatives · País</p>
          ...
          <p className="mt-4 max-w-xl font-body text-base text-paper/70">
            {group.items.length} {group.items.length === 1 ? "compañía" : "compañías"} visitadas en{" "}
            {group.label}.
          </p>
        </div>
      </section>
